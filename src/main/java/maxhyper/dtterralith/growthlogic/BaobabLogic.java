...
public class BaobabLogic extends VariateHeightLogic {
	...
	protected GrowthLogicKitConfiguration createDefaultConfiguration() {
		return super.createDefaultConfiguration().with(HEIGHT_VARIATION, 4);
	}
	...
	public int[] populateDirectionProbabilityMap(GrowthLogicKitConfiguration configuration,
		...
		if (!signal.isInTrunk()) {
			Direction relativePosToRoot = Direction.getNearest(signal.delta.getX(), 0, signal.delta.getY(), originDir);
			...
			boolean isBranchUp = world.getBlockState(pos.relative(relativePosToRoot))
