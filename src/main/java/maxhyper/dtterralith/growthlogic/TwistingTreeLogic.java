...
public class TwistingTreeLogic extends GrowthLogicKit {
	...
	public static final ConfigurationProperty<Float> CHANCE_TO_SPLIT = ConfigurationProperty
			.floatProperty("chance_to_split");
	public static final ConfigurationProperty<Integer> DOWN_PROBABILITY = ConfigurationProperty
			.integer("down_probability");
	...
	protected GrowthLogicKitConfiguration createDefaultConfiguration() {
		return super.createDefaultConfiguration().with(CHANCE_TO_SPLIT, 0.01f).with(DOWN_PROBABILITY, 0)
				.with(HEIGHT_VARIATION, 3).with(SPLIT_ENDS_ENERGY, 3);
	}

	@Override
	protected void registerProperties() {
		this.register(CHANCE_TO_SPLIT, DOWN_PROBABILITY, HEIGHT_VARIATION, SPLIT_ENDS_ENERGY);
	}
	...
	public int[] populateDirectionProbabilityMap(GrowthLogicKitConfiguration configuration,
		...
		for (Direction direction : Direction.values()) {
			...
			probMap[direction.ordinal()] = rad
					+ (level.getRandom().nextFloat() < configuration.get(CHANCE_TO_SPLIT) ? 1 : 0);
		}
		// If there are not enough valid branches, just allow any direction except down
		if (count <= 1 || signal.energy < configuration.get(SPLIT_ENDS_ENERGY)) {
	...
	public float getEnergy(GrowthLogicKitConfiguration configuration, PositionalSpeciesContext context) {
		...
		return super.getEnergy(configuration, context)
				* context.species().biomeSuitability(context.level(), context.pos())
				+ (CoordUtils.coordHashCode(context.pos().above(month), 3) % configuration.get(HEIGHT_VARIATION)); // Vary
																													// the
																													// height
																													// energy
																													// by
