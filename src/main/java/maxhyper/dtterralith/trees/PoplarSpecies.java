package maxhyper.dtterralith.trees;
		...
		public boolean returnRun(BlockState state, LevelAccessor level, BlockPos pos, Direction fromDir) {
			...
			if (branch != null) {
					...
					if (!dir.equals(fromDir)) { // Don't count where the signal originated from
						...
						if (dPos.equals(last)) { // or the branch we just came back from
							isTwig = false; // on the return journey if the block we just came from is a branch we are
											// obviously not the endpoint(twig)
							isTop = false;
							continue;
						}
				...
				if (isTop) {
					...
					SimpleVoxmap leafCluster = DTTerralithLeafClusters.POPLAR_TOP;
					...
					// The new branch should be the square root of all of the sums of the areas of
					// the branches coming into it.
					radius = (float) Math.sqrt(areaAccum)
